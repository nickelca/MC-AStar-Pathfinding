...
execute as @e[tag=pfind,tag=new] at @s run function pathfinding:calculate_cost
...
scoreboard players operation $lowest pfind.var < @e[tag=explore,tag=!used] pfind.cost
execute as @e[tag=pfind,tag=explore,tag=!used] if score @s pfind.cost = $lowest pfind.var at @s run function pathfinding:check_end